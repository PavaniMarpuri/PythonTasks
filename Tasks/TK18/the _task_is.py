...
● The email class should contain the following class variable and default
value:
    ○ has_been_read - initialised to False.
● The Email class should also contain the following class method to
...
● Create the following functions to add functionality to your email simulator:
    ○ populate_inbox() - a function which creates an email object with the
    ...
    ○ list_emails() - a function that loops through the inbox and prints
        each email’s subject_line, along with a corresponding number. For
...
when the user chooses to:
    ...
    3. Quit application
Note: menu option 2 does not require a function. Access the
corresponding class variable to retrieve the subject_line only.
"""
